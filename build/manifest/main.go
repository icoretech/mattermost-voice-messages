...
export const clientTranscriptionQuantizationSet =
  new Set<ClientTranscriptionQuantization>(clientTranscriptionQuantizations);
...
func applyManifest(manifest *model.Manifest) error {
	...
	if manifest.HasWebapp() {
		...
		modelOptions, err := clientTranscriptionSettingOptions(manifest, "ClientTranscriptionModel")
		if err != nil {
			return err
		}
		...
		quantizationOptions, err := clientTranscriptionSettingOptions(manifest, "ClientTranscriptionQuantization")
		if err != nil {
			return err
		}
...
func clientTranscriptionSettingOptions(manifest *model.Manifest, key string) ([]string, error) {
	settingsSchema := manifest.SettingsSchema
	if settingsSchema == nil {
		return nil, errors.Errorf("%s requires settings schema options", key)
	}

	for _, setting := range settingsSchema.Settings {
		...
		if len(options) == 0 {
			return nil, errors.Errorf("%s has no options", key)
		}
		return options, nil
	}
